...
describe('Event', () => {
  ...
  test('Fetch all events', async () => {
    ...
    for (const event of events) {
      ...
      } else if (event.key === 'recipient.destroy') {
        expect(() => Event.OpnPaymentsDeletedRecipientEventSchema.parse(event)).not.toThrowError()
      ...
      } else if (event.key === 'transfer.destroy') {
        expect(() => Event.OpnPaymentsDeletedTransferEventSchema.parse(event)).not.toThrowError()
